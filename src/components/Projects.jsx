...
const ProjectCard = ({ project, index }) => {
    ...
    return (
        <motion.div
            ...
            className="group relative bg-white dark:bg-[#111] rounded-[20px] overflow-hidden border border-slate-200 dark:border-white/5 shadow-md dark:shadow-xl hover:shadow-xl dark:hover:shadow-[#00AEEF]/20 hover:border-[#00AEEF]/50 transition-all duration-500 flex flex-col h-full"
        >
            {/* Category Badge */}
            ...
            <div 
                className="relative h-64 overflow-hidden cursor-pointer bg-slate-100 dark:bg-gray-900"
                onClick={() => window.open(project.live, '_blank')}
            >
                {/* Skeleton Loader */}
                {!imageLoaded && !hasError && (
                    <div className="absolute inset-0 animate-pulse bg-slate-200 dark:bg-white/5 flex items-center justify-center">
                        <Code className="text-slate-400 dark:text-white/20" size={32} />
                    </div>
                )}
                ...
                <div className="absolute inset-0 bg-slate-900/80 dark:bg-black/70 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center p-6 text-center z-20">
                    <motion.div
                    ...
                    >
                        ...
                        <p className="text-gray-100 dark:text-gray-200 text-sm leading-relaxed max-w-xs">
                            {project.desc}
                        </p>
                        ...
                        {project.tech && (
                            <div className="flex flex-wrap justify-center gap-2 mt-2">
                                {project.tech.map((t) => (
                                    <span key={t} className="text-[10px] uppercase tracking-wider font-bold px-2 py-1 bg-white/20 dark:bg-white/10 rounded-md text-white dark:text-gray-300 border border-white/10">
                                        {t}
                                    </span>
                                ))}
                            </div>
            ...
            <div className="p-6 flex flex-col flex-grow">
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2 group-hover:text-[#00AEEF] transition-colors leading-tight">
                    {project.title}
                </h3>
                ...
                <div className="grid grid-cols-2 gap-3 mt-auto">
                    <a
                        ...
                        className="flex items-center justify-center gap-2 py-2 rounded-xl border border-slate-300 dark:border-white/10 text-slate-800 dark:text-white hover:bg-slate-900 hover:text-white dark:hover:bg-white dark:hover:text-black transition-all duration-300 font-medium text-sm"
                    >
                        <Github size={16} />
                        <span>GitHub</span>
                    </a>
                    ...
                    {project.youtube && (
                        <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="col-span-2 flex items-center justify-center gap-2 py-2 rounded-xl border border-red-500/30 text-slate-800 dark:text-white hover:bg-red-500 hover:text-white transition-all duration-300 font-medium text-sm group/yt"
                        >
...
const Projects = () => {
    return (
        <section id="projects" className="min-h-screen py-20 relative bg-slate-50 dark:bg-black transition-colors duration-500 overflow-hidden">
            <div className="container mx-auto px-12 md:px-24 lg:px-32 relative z-10">
                ...
                <div className="mt-16 text-center">
                    <motion.a
                        ...
                        whileTap={{ scale: 0.95 }}
                        className="inline-block px-8 py-3 rounded-full border border-slate-300 dark:border-gray-700 text-slate-700 dark:text-gray-300 hover:border-[#00AEEF] hover:text-[#00AEEF] transition-all hover:shadow-[0_0_20px_rgba(0,174,239,0.2)]"
                    >
                        View More on GitHub
                    </motion.a>
                </div>
